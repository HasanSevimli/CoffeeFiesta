      ...
      <div class="col-md-12 text-center">
        <div class="table-responsive" style="margin-top: 20px;">
          ...
          try {
            ...
            echo "<thead><tr><th>ID</th><th>Çeşit</th><th>Köken</th><th>Kavurma Derecesi</th><th>Lezzet Profili</th><th>Aroma Profili</th>";
            ...
            while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
              echo "<tr><td>" . $row['ID'] . "</td><td>" . $row['Cesit'] . "</td><td>" . $row['Koken'] . "</td><td>" . $row['Kavurma_Derecesi'] . "</td><td>" . $row['Lezzet_Profili'] . "</td><td>" . $row['Aroma_Profili'] . "</td>";

              if ($usertype == 1) {
                echo "<td class='action-buttons'><button class='btn btn-danger btn-sm' onclick='sil(" . $row['ID'] . ")'>Sil</button></td>";
              }
      ...
      function sil(ID) {
        if (confirm("Bu ürünü silmek istediğinizden emin misiniz?")) {
          $.ajax({
            type: "POST",
            url: "iptal.php",
            data: {id: ID}, // Değişiklik burada yapıldı
